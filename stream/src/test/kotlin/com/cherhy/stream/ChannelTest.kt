...
class ChannelTest : FunSpec({
    ...
    test("buffer가 없는 Channel에서 데이터를 소비하면 buffer가 없기 때문에 데이터 1개씩만 소비 되고 1개씩 소비된다") {
    ...
    test("buffer가 없는 Channel에서 구독을 하지 않고 데이터를 송신하면 소비할 때 까지 무한 대기한다") {
        val rendezvousChannel = Channel<Int>()
    ...
    test("buffer가 무한한 Channel에서 데이터 소비하면 buffer가 무한하기 때문에 데이터가 1번에 모두 소비된다") {
    ...
    test("채널이 닫히지 않은 상태에서 소비를 하면 소비자는 계속해서 새로운 데이터가 온다고 생각해서 채널이 닫힐 때까지 대기한다") {
        val unlimitedChannel = Channel<Int>(UNLIMITED)
